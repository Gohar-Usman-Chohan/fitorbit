...
export default function TrainerClientsContent() {
  ...
  useEffect(() => {
    ...
    async function loadClients() {
      try {
        ...
        if (!cancelled) {
          setClients(list.map(mapTrainerClient));
          setError(null);
        }
      } catch (err: any) {
        if (!cancelled) {
      ...
      {clients.length > 0 ? (
              ...
              <article key={client.id} className="card card-body space-y-3">
                ...
                {client.planTitle !== 'Not assigned' ? (
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-600">{client.progress}% complete</p>
                  </div>
                ) : null}
                {client.planTitle !== 'Not assigned' ? (
                  <Link
                    href={`/trainer/progress-reports?clientId=${client.id}`}
        ...
        <div className="rounded-lg bg-white p-8 text-center shadow sm:p-12">
          ...
          <p className="mt-2 text-gray-400">
            Clients appear here after they book you or you assign them a workout plan.
          </p>
        </div>
      )}
    </div>
