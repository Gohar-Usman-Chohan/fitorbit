    ...
    <ChatLayout
      ...
      conversation={
        selectedConversation ? (
                    ...
                    >
                      <div
                        className={`max-w-[85%] rounded-lg p-3 sm:max-w-xs ${
                          isOwn ? 'bg-orange-600 text-white' : 'bg-gray-200 text-gray-800'
                        }`}
                      >
                        <p>{message.content || message.messageContent}</p>
                        <p className={`mt-1 text-xs ${isOwn ? 'text-orange-100' : 'text-gray-600'}`}>
            ...
            composer={
              <div className="flex flex-col gap-2 sm:flex-row">
                <input
                  ...
                  disabled={isSending}
                  className="flex-1 rounded-lg border border-gray-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500 disabled:bg-gray-100"
                />
                <button
                  onClick={handleSendMessage}
                  disabled={isSending || !messageText.trim()}
                  className="rounded-lg bg-orange-600 px-6 py-2 text-sm font-semibold text-white transition hover:bg-orange-700 disabled:bg-orange-400 sm:shrink-0"
                >
          ...
          <ChatEmptyState>
            <p className="text-gray-500">Select a client chat to start messaging</p>
          </ChatEmptyState>
        )
      }
    />
